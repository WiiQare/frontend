                ...
                <div className="flex flex-col gap-1">
                    ...
                    <span className="flex gap-3 items-center text-sm text-gray-700 font-light mt-3">
                        <span className="bg-orange opacity-90 px-2 py-1 text-white text-xs rounded-md w-fit flex gap-3 items-center">
                            {row.packages ? row.packages.length : 0} Plan{row.packages.length > 1 ? "s" : ""}
                        </span>
                    </span>
    ...
    
    {
        ...
        selector: row => (
            <div className="flex flex-col gap-4">
                <span className={`bg-gray-300 text-gray-900 py-1 pl-2 font-semibold w-fit pr-4 rounded-full opacity-80 `}>{businessTypes[row.businessType].text}</span>
                <div className="flex flex-col gap-1">
                    <Link href={`tel:${row.phone}`} legacyBehavior>
                        <span className="text-xs text-gray-500 flex items-center gap-1 cursor-pointer"><FcCellPhone size={18}/> {row.phone}</span>
                    </Link>

                    <Link href={`mailto:${row.email}`} legacyBehavior>
                        <span className="text-xs text-gray-500 flex items-center gap-1 cursor-pointer"><FcAddressBook size={18}/> {row.email}</span>
                    </Link>
...
const Hospital = () => {
    ...
    const { data, isLoading, isError } = Fetcher(`/provider/list`, session.user.data.access_token);
