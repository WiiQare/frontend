...
const DetailsTransaction = ({ data, isLoading, isError, slug, fullname, email, phone }) => {
  ...
  useEffect(() => {
    ...
    if (data && !data.code) {
      const somme = data && data?.operations.reduce(
      ...
      console.log(month);
      const price = data?.amount / (data?.frequency == "MONTH" ? month : data?.week == "WEEK" ? month * 4 : month * 31)
            ...
            <div className="w-full overflow-hidden md:col-span-2 rounded-lg py-8 flex flex-col gap-6 bg-white drop-shadow-sm">
              <div className="px-8 space-y-8">
                <div>
                  ...
                  <h2 className="font-semibold text-3xl">#{slug.split("-")[0].toUpperCase()}</h2>
                </div>
                <div className="flex flex-col gap-4">
                  <div className="overflow-x-auto">
                    <table className="table table-zebra text-xs w-full flex items-center">
                      ...
                      {
                        !data || !data?.operations || data?.operations?.length == 0 ? (
                          //<img src="https://i.goopics.net/z4ix58.png" alt="" className='opacity-60 w-20' />
                          <p className='text-center w-full py-2 text-gray-400 text-md'>Aucun versement...</p>
                        ) : (
                          <tbody>
                            {
                              data.operations.map((operation, index) => (
                                <tr key={index}>
                                  <th>#{operation.id.split("-")[operation.id.split("-").length - 1].toUpperCase()}</th>
                  ...
                  <span className="w-full bg-gray-100 text-gray-500 h-fit p-2 rounded-lg flex gap-2 items-start text-sm">
                    ...
                    <ul className="leading-6 font-light w-full pr-5 space-y-2">
                      <li className='flex justify-between w-full'>
                        Objectif à atteindre: <span className='text-primary font-bold'>{new Intl.NumberFormat('en-US', {
                          style: 'currency',
                      ...
                      <li className='flex justify-between w-full'>
                        Frequence de paiement: <span className=''>{new Intl.NumberFormat('en-US', {
                          style: 'currency',
                          currency: data?.currency?.toLowerCase() ?? "USD",
                        }).format(count.price.toFixed(2))} /{data.frequency == "MONTH" ? "mois" : data.frequency == "WEEK" ? "semaine" : "jour"}</span>
                      </li>

                      <hr />

                      <li className='flex justify-between w-full'>
                        Rester à atteindre: <span className='font-bold'>{new Intl.NumberFormat('en-US', {
                          style: 'currency',
                          currency: data?.currency?.toLowerCase() ?? "USD",
                        }).format(data.amount - count.somme)}</span>
                      </li>
                    </ul>
                  </span>
                </div>
                      ...
                      <div className="flex justify-around">
                        ...
                        <div className="">
                          <Link href={`/saving/operation/${slug}`} legacyBehavior >
                            ...
                            <a className={`flex items-center gap-2 justify-between rounded-lg bg-orange py-2 px-3 effect-up shadow`}
                            >
                              <span className="bg-white text-sky p-1 rounded-md">
                                <HiOutlinePlus />
                              </span>
                              <span className="text-sm font-light">Récharger</span>
                            </a>
                          </Link>
                        </div>
                      </div>
              ...
              <div className="flex items-center justify-center min-h-full p-4 text-center">
                ...
                >
                  <Dialog.Panel className="w-full max-w-md p-6 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-xl space-y-10">
                    <div className="flex flex-col gap-6">
                      Lorem ipsum dolor sit, amet consectetur adipisicing elit. Voluptatibus amet, ex at qui voluptate consequatur iste reprehenderit voluptatum culpa quae vitae, laboriosam minima placeat, odit ullam corrupti sunt suscipit quis?
                    </div>
                  </Dialog.Panel>
                </Transition.Child>
              </div>
