...
function ResetForm() {
	...
	const newChangePassword = useMutation(resetPassword, {
		onSuccess: (res) => {
			...
			if (res.code) {
				...
				setState({ type: 1, message: "Mot de passe réinitialoser avec succès" })
	...
	const ValidationSchema = yup.object().shape({
		...
		confirmPassword: yup.string().required("Veuillez confirmer votre mot de passe").oneOf([yup.ref('password'), null], 'Le deux mot de passe ne correspondent pas'),
	});
							...
							<div className="md:w-3/4 mx-auto">
								...
								<form id="signinform" className="" onSubmit={formik.handleSubmit}>
									<Stack spacing={2}>
										...
										{formik.errors.password  && formik.touched.password ? renderError(formik.errors.password) : <></>}
										...
										{formik.errors.confirmPassword  && formik.touched.confirmPassword? renderError(formik.errors.confirmPassword) : <></>}

										<Box>
											<Button size="large" variant="contained" type="submit" className="disabled:bg-gray-200">
												{newChangePassword.isLoading ? <LoadingButton /> : 'Changer le mot de passe'}
											</Button>
										</Box>
									</Stack>
								</form>
