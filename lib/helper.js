...
export async function invitationFriend(formData) {
    try {
        ...
        const json = response.status == 201 ? { state: true } : await response.json();
...
export async function sendSMSHash(formData) {
    try {
        ...
        console.log(response);
        const json = response.status == 200 || response.status == 201 ? response : await response.json();
...
export async function addPlan(formData) {
    try {
        ...
        const response = await fetch("https://api.wiiqare-app.com/api/v1/savings", Options);
        const json = await response.json();
...
export async function authlogic() {
    try {
        const Options = {
            ...
            body: JSON.stringify(
                {
                    "userKey": "WR26jWceSVW3IND0z1BLd0Hn",
        ...
        const response = await fetch("https://sandbox.authologic.com/api/conversations", Options);
